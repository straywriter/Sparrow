...
static const char *s_attrName[] = {
    "P",  "Attrib::Position",  "N",  "Attrib::Normal",    "T",  "Attrib::Tangent",   "B",  "Attrib::Bitangent",
    "C0", "Attrib::Color0",    "C1", "Attrib::Color1",    "C2", "Attrib::Color2",    "C3", "Attrib::Color3",
    "I",  "Attrib::Indices",   "W",  "Attrib::Weights",   "T0", "Attrib::TexCoord0", "T1", "Attrib::TexCoord1",
    "T2", "Attrib::TexCoord2", "T3", "Attrib::TexCoord3", "T4", "Attrib::TexCoord4", "T5", "Attrib::TexCoord5",
    "T6", "Attrib::TexCoord6", "T7", "Attrib::TexCoord7",
};
...
static AttribToId s_attribToId[] = {
    ...
    {Attrib::Position, 0x0001},  {Attrib::Normal, 0x0002},    {Attrib::Tangent, 0x0003},   {Attrib::Bitangent, 0x0004},
    {Attrib::Color0, 0x0005},    {Attrib::Color1, 0x0006},    {Attrib::Color2, 0x0018},    {Attrib::Color3, 0x0019},
    {Attrib::Indices, 0x000e},   {Attrib::Weight, 0x000f},    {Attrib::TexCoord0, 0x0010}, {Attrib::TexCoord1, 0x0011},
    {Attrib::TexCoord2, 0x0012}, {Attrib::TexCoord3, 0x0013}, {Attrib::TexCoord4, 0x0014}, {Attrib::TexCoord5, 0x0015},
    {Attrib::TexCoord6, 0x0016}, {Attrib::TexCoord7, 0x0017},
};
...
static AttribTypeToId s_attribTypeToId[] = {
    ...
    // added.
    {AttribType::Uint8, 0x0001}, {AttribType::Uint10, 0x0005}, {AttribType::Int16, 0x0002},
    {AttribType::Half, 0x0003},  {AttribType::Float, 0x0004},
};
                ...
                void *_data, uint32_t _index)
{
    ...
    case AttribType::Uint8: {
        ...
        if (_inputNormalized)
        {
            if (asInt)
            {
                ...
                {
                default:
                    *packed++ = uint8_t(*_input++ * 127.0f + 128.0f);
                    BX_FALLTHROUGH;
                case 3:
                    *packed++ = uint8_t(*_input++ * 127.0f + 128.0f);
                    BX_FALLTHROUGH;
    ...
    case AttribType::Int16: {
        ...
        if (_inputNormalized)
        {
            if (asInt)
            {
                ...
                {
                default:
                    *packed++ = int16_t(*_input++ * 32767.0f);
                    BX_FALLTHROUGH;
                case 3:
                    *packed++ = int16_t(*_input++ * 32767.0f);
                    BX_FALLTHROUGH;
            ...
            else
            {
                ...
                {
                default:
                    *packed++ = int16_t(*_input++ * 65535.0f - 32768.0f);
                    BX_FALLTHROUGH;
                case 3:
                    *packed++ = int16_t(*_input++ * 65535.0f - 32768.0f);
                    BX_FALLTHROUGH;
    ...
    case AttribType::Half: {
        ...
        {
        default:
            *packed++ = bx::halfFromFloat(*_input++);
            BX_FALLTHROUGH;
        case 3:
            *packed++ = bx::halfFromFloat(*_input++);
            BX_FALLTHROUGH;
...
void vertexUnpack(float _output[4], Attrib::Enum _attr, const VertexLayout &_layout, const void *_data, uint32_t _index)
{
    ...
    case AttribType::Uint8: {
        ...
        if (asInt)
        {
            ...
            {
            default:
                *_output++ = (float(*packed++) - 128.0f) * 1.0f / 127.0f;
                BX_FALLTHROUGH;
            case 3:
                *_output++ = (float(*packed++) - 128.0f) * 1.0f / 127.0f;
                BX_FALLTHROUGH;
        ...
        else
        {
            ...
            {
            default:
                *_output++ = float(*packed++) * 1.0f / 255.0f;
                BX_FALLTHROUGH;
            case 3:
                *_output++ = float(*packed++) * 1.0f / 255.0f;
                BX_FALLTHROUGH;
    ...
    case AttribType::Int16: {
        ...
        if (asInt)
        {
            ...
            {
            default:
                *_output++ = float(*packed++) * 1.0f / 32767.0f;
                BX_FALLTHROUGH;
            case 3:
                *_output++ = float(*packed++) * 1.0f / 32767.0f;
                BX_FALLTHROUGH;
        ...
        else
        {
            ...
            {
            default:
                *_output++ = (float(*packed++) + 32768.0f) * 1.0f / 65535.0f;
                BX_FALLTHROUGH;
            case 3:
                *_output++ = (float(*packed++) + 32768.0f) * 1.0f / 65535.0f;
                BX_FALLTHROUGH;
    ...
    for (uint32_t ii = 0; ii < Attrib::Count; ++ii)
    {
        ...
        if (_destLayout.has(attr))
        {
            ...
            if (_srcLayout.has(attr))
            {
                ...
                cop.op = _destLayout.m_attributes[attr] == _srcLayout.m_attributes[attr] ? ConvertOp::Copy
                                                                                         : ConvertOp::Convert;
            }
            else
            {
...
static IndexT weldVerticesRef(IndexT *_output, const VertexLayout &_layout, const void *_data, uint32_t _num,
                              float _epsilon)
{
...
template <typename IndexT>
static IndexT weldVertices(IndexT *_output, const VertexLayout &_layout, const void *_data, uint32_t _num,
