...
struct Context
{
    ...
    Context()
        : m_render(&m_frame[0]), m_submit(&m_frame[BGFX_CONFIG_MULTITHREADED ? 1 : 0]),
          ...
          m_rtMemoryUsed(0), m_textureMemoryUsed(0), m_renderCtx(NULL), m_renderMain(NULL), m_renderNoop(NULL),
          m_rendererInitialized(false), m_exit(false), m_flipAfterRender(false), m_singleThreaded(false)
    {
    }
        ...
        for (uint16_t ii = 0, num = m_textureHandle.getNumHandles(); ii < num; ++ii)
        {
            ...
            if (BackbufferRatio::Count != textureRef.m_bbRatio)
            {
                ...
                resizeTexture(handle, uint16_t(m_init.resolution.width), uint16_t(m_init.resolution.height),
                              textureRef.m_numMips, textureRef.m_numLayers);
                                              ...
                                              BackbufferRatio::Enum _ratio, bool _immutable))
    {
        ...
        ref.init(_ratio, _info->format, _info->storageSize, imageContainer.m_numMips, imageContainer.m_numLayers,
                 0 != (g_caps.supported & BGFX_CAPS_TEXTURE_DIRECT_ACCESS), _immutable,
    ...
    bool checkFrameBuffer(uint8_t _num, const Attachment *_attachment) const
    {
        ...
        return color <= g_caps.limits.maxFBAttachments && depth <= 1;
    }
                                                      ...
                                                      bool _destroyTextures))
    {
        ...
        BX_ASSERT(checkFrameBuffer(_num, _attachment),
                  "Too many frame buffer attachments (num attachments: %d, max color attachments %d)!", _num,
