...
inline PlatformData::PlatformData() : ndt(NULL), nwh(NULL), context(NULL), backBuffer(NULL), backBufferDS(NULL)
{
}
...
inline Resolution::Resolution()
    : format(TextureFormat::RGBA8), width(1280), height(720), reset(BGFX_RESET_NONE), numBackBuffers(2),
      maxFrameLatency(0)
{
}
...
inline Init::Limits::Limits()
    : maxEncoders(BGFX_CONFIG_DEFAULT_MAX_ENCODERS), minResourceCbSize(BGFX_CONFIG_MIN_RESOURCE_COMMAND_BUFFER_SIZE),
...
inline Init::Init()
    : type(RendererType::Count), vendorId(BGFX_PCI_ID_NONE), deviceId(0), debug(BX_ENABLED(BGFX_CONFIG_DEBUG)),
      profile(BX_ENABLED(BGFX_CONFIG_DEBUG_ANNOTATION)), callback(NULL), allocator(NULL)
{
}
...
struct Caps
{
    ...
    ///
    /// @attention See BGFX_CAPS_* flags at https://bkaradzic.github.io/Render/Render.html#available-caps
    ///
    uint64_t supported;
...
struct Attachment
{
    ...
    ///
    void init(TextureHandle _handle, Access::Enum _access = Access::Write, uint16_t _layer = 0, uint16_t _mip = 0,
...
inline void Attachment::init(TextureHandle _handle, Access::Enum _access, uint16_t _layer, uint16_t _mip,
                             uint8_t _resolve)
{
    access = _access;
    handle = _handle;
