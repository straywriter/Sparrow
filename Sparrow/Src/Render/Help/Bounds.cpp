...
bool overlap(const Disk &_diskA, const Disk &_diskB)
{
    ...
    return bx::sqrt(square(_diskA.radius) - square(lenA)) + bx::sqrt(square(_diskB.radius) - square(lenB)) >=
           distance(pa, pb);
}
...
template <typename Ty> bool overlap(const Triangle &_triangle, const Ty &_ty)
{
    Plane plane;
    calcPlane(plane, _triangle);
