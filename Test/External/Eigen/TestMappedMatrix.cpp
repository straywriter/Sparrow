...
#define ASSERT_APPROX(left, right) ASSERT_TRUE((left).isApprox(right))
...
template <typename MatrixType> void map_class_matrix(const MatrixType &m)
{
    ...
    EXPECT_EQ(map1, MatrixType::Ones(rows, cols));
    EXPECT_EQ(map2, MatrixType::Ones(rows, cols));
    EXPECT_EQ(map3, MatrixType::Ones(rows, cols));
    ...
    ASSERT_APPROX(s1 * map1, s1 * map2);
    ASSERT_APPROX(s1 * ma1, s1 * ma2);
    EXPECT_EQ(s1 * ma1, s1 * ma3);
    ASSERT_APPROX(s1 * map1, s1 * map3);

    map2 *= s1;
    map3 *= s1;
    ASSERT_APPROX(s1 * map1, map2);
    ASSERT_APPROX(s1 * map1, map3);

    if (size <= 256)
    {
        EXPECT_EQ(map4, MatrixType::Ones(rows, cols));
        map4 = map1;
        MatrixType ma4 = map4;
        EXPECT_EQ(map1, map4);
        EXPECT_EQ(ma1, map4);
        EXPECT_EQ(ma1, ma4);
        ASSERT_APPROX(s1 * map1, s1 * map4);

        map4 *= s1;
        ASSERT_APPROX(s1 * map1, map4);
    }
...
template <typename PlainObjectType> void check_const_correctness(const PlainObjectType &)
{
    ...
    ASSERT_TRUE(!(internal::traits<Map<ConstPlainObjectType>>::Flags & LvalueBit));
    ASSERT_TRUE(!(internal::traits<Map<ConstPlainObjectType, AlignedMax>>::Flags & LvalueBit));
    ASSERT_TRUE(!(Map<ConstPlainObjectType>::Flags & LvalueBit));
    ASSERT_TRUE(!(Map<ConstPlainObjectType, AlignedMax>::Flags & LvalueBit));
}
...
TEST(VectorMap, ALL)
{
  ...
  for(int i = 0; i < 10; i++)
  {
    ...
    map_not_aligned_on_scalar<double>();
