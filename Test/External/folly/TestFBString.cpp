...
// template <class Integral1, class Integral2> Integral2 random(Integral1 low, Integral2 up)
// {
...
// // #include <unistd.h>
// // no runtime random
// template <class String> void randomString(String *toFill, unsigned int maxSize = 1000)
// {
//     assert(toFill);
...
// // using namespace std;
// // const char possible_characters[] = "0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz";
// //     random_device rd;
// //     mt19937 engine(rd());
// //     std::uniform_int_distribution<int> disti(1,maxSize);
// //     uniform_int_distribution<> dist(0, 62-1);
// // int max_length=disti(engine);
// //    String ret = "";
// //     for(int i = 0; i < max_length; i++){
// //         int random_index = dist(engine); //get index between 0 and possible_characters.size()-1
// //         ret +=(String::T) possible_characters[random_index];
// //     }
// //     *toFill=ret;
...
// template <class String, class Integral> void Num2String(String &str, Integral n)
// {
...
//     String test1;
//     randomString(&test1);
//     assert(test1.size() == char_traits<typename String::value_type>::length(test1.c_str()));
//     auto len = test.size();
//     test += test1;
...
//     EXPECT_EQ(char_traits<typename String::value_type>::length(test.c_str()), len);
//     test += test;
//     // test.append(test);
//     EXPECT_EQ(test.size(), 2 * len);
//     EXPECT_EQ(char_traits<typename String::value_type>::length(test.c_str()), 2 * len);
...
//     EXPECT_EQ(char_traits<typename String::value_type>::length(test.c_str() + pos), len - pos);
//     if (avoidAliasing)
//     {
...
//     typename String::iterator res = test.insert(test.begin() + pos, random('a', 'z'));
...
//         test.replace(test.begin() + pos, test.begin() + pos + random(0, test.size() - pos), newString);
//     }
//     else
//     {
//         test.replace(test.begin() + pos, test.begin() + pos + random(0, test.size() - pos), test);
//     }
//     pos = random(0, test.size());
//     if (avoidAliasing)
//     {
//         auto newString = String(test);
//         test.replace(test.begin() + pos, test.begin() + pos + random(0, test.size() - pos), newString.c_str(),
//                      test.size() - random(0, test.size()));
//     }
//     else
//     {
//         test.replace(test.begin() + pos, test.begin() + pos + random(0, test.size() - pos), test.c_str(),
//                      test.size() - random(0, test.size()));
//     }
//     pos = random(0, test.size());
...
//     pos = random(0, test.size());
//     test.replace(test.begin() + pos, test.begin() + pos + random(0, test.size() - pos), random(0, maxString),
//                  random('a', 'z'));
// }
...
// // auto l = [&](const char *const clause, void (*f_string)(std::string &), void (*f_fbstring)(folly::fbstring &),
...
//             } while (++count % 1 != 0);                                                                                \
//         };                                                                                                             \
//         l(#x, clause11_##x<std::string>, clause11_##x<folly::fbstring>, clause11_##x<folly::basic_fbstring<wchar_t>>); \
//     }
...
// //             wc = folly::basic_fbstring<wchar_t>(wr.c_str());
// // #endif
// //             int localSeeda = seed + count;
// //             //   rng = std::mt19937(localSeeda);
// //             f_string(r);
// //             //   rng = std::mt19937(localSeeda);
// //             f_fbstring(c);
// //             EXPECT_EQ(r, c) << "Lengths: " << r.size() << " vs. " << c.size() << "\nReference: '" << r << "'"
...
//     };
//     EXPECT_EQ(str.capacity(), goodMallocSize(3840) - sizeof(DummyRefCounted) - sizeof(char));
// }
...
//     ss << std::right << setw(6) << fbstr;
//     EXPECT_EQ(ss.str(), " Hello");
//     ss.str("");

//     ss << setw(6) << left << fbstr;
//     EXPECT_EQ(ss.str(), "Hello ");
//     ss.str("");

//     ss << std::setw(6) << std::right << fbstr;
//     EXPECT_EQ(ss.str(), " Hello");
//     ss.str("");
...
//     ss << std::setw(6) << std::setfill('^') << fbstr;
//     EXPECT_EQ(ss.str(), "^Hello");
//     ss.str("");

//     std::string aa;
//     aa = toStdString(fbstr);

//     std::cout << aa;
//     fbstr = "abcd";
//     std::cout << aa;

//     //  std::cout<<std::setw(10)<<std::setfill('*')<<fbstr<<std::endl;
// }
...
// TEST(FBStringMy, test)
// {
//     fbstring a("abcdef");
//     std::cout << a.capacity();
//     std::string randoms;
//     randomString(&randoms);
//     fbstring r2=randoms;

//     std::cout << randoms<<std::endl;
//     std::cout << r2<<std::endl;
//     EXPECT_EQ(r2,randoms);

// //     std::string rr1=randoms;
// //     fbstring rr2=randoms;

// //     clause11_21_4_6_1(rr1);
// //     clause11_21_4_6_1(rr2);

// // std::cout<<rr1<<std::endl;
// // std::cout<<rr2<<std::endl;

// fbstring ft("abcdefgh");
// std::cout<<ft.find('c')+1;

// std::string rant;
// randomstdString(&rant);
// std::cout<<std::endl<<rant;
// std::cout<<std::endl<<rant[0];
// }