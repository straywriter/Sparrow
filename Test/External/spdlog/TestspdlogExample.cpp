...
{
    spd::set_error_handler([](const std::string &msg) { std::cerr << "my err handler: " << msg << std::endl; });
...
void example()
{
    try
    {
        ...
        spd::apply_all([&](std::shared_ptr<spd::logger> l) { l->info("End of example."); });
...
TEST_CASE("testspdlogexample1", "1")
{
    example();
    syslog_example();
}

// TEST_CASE("testspdlogexample2", "2")
// {
//     // example();
//     // user_defined_example();
// }

TEST_CASE("testspdlogexample3", "3")
{
    err_handler_example();
}
